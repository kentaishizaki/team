...
<html lang = "ja">
	...
	<head>
		...
		<title>チーム連携システム・新規メッセージ送信完了</title>
	...
	try {
		...
		if ( !( $userInfo = $data1 -> fetch () ) ) {
			...
			if ( !( $commonData = $data2 -> fetch () ) ) {
				...
				// SQL文のプリコンパイル
				$data = $db -> prepare ( "insert into messageContents ( message, userId, leaderId, insertDate ) values ( ?, ?, ?, ? ); ");

				// プレースホルダに値を代入
				$data -> bindValue ( 1, htmlspecialchars ( $_POST['message'] ) );
				$data -> bindValue ( 2, htmlspecialchars ( $_POST['userId'] ) );
				$data -> bindValue ( 3, htmlspecialchars ( $_POST['leaderId'] ) );
				$data -> bindValue ( 4, $insertDate );
